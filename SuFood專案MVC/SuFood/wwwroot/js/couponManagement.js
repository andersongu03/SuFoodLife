const vm = new Vue({
	...
	methods: {
		...
		CreateCoupon() {
			...
			axios.post('/BackStage/CouponManagement/CreateCoupon', this.manage, {
				headers: {
		...
		EditCoupon() {
			...
			axios.post("/BackStage/CouponManagement/EditCoupon/", tempData, {
				headers: {
					'Content-Type': 'application/json'
				}
			}).then(response => {
		...
		DelCoupon(id) {
			...
			axios.delete(`/BackStage/CouponManagement/DeleteCoupon/${id}`)
				.then(response => {
					this.toast = response.data
					this.closePopupShowHint()
					_this.GetCouponInfo();
